...
gulp.task('packages:ci', async () => {
  ...
  for (let i = 0; i < packages.length; i++) {
    ...
    if (!currentPackage.includes('python-docker-isolation-provider') && !currentPackage.includes('workflows')) parts.push(`echo "${currentPackage} npm ci" && cd ${__dirname}${delimetr}${currentPackage} && npm ci`);
  }
