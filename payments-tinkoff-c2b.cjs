...
const PACKAGE_NAME = "@deep-foundation/payments-tinkoff-c2b";
...
const installPackage = async () => {
  ...
  try {
    ...
    const generateToken = ${generateTokenStringWithInsertedTerminalPassword};
      ...
      const tinkoffProviderTypeLinkId = await deep.id("${PACKAGE_NAME}", "TinkoffProvider");
      ...
      const {data: mpDownPay, error: mpDownPaySelectQueryError} = await deep.select({
        down: {
          ...
          tree_id: { _eq: await deep.id("${PACKAGE_NAME}", "paymentTree") },
        },
      });
      console.log({mpDownPay});
      if(mpDownPaySelectQueryError) { throw new Error(mpDownPaySelectQueryError.message); }
    
      const paymentTypeLinkId = await deep.id("${PACKAGE_NAME}", "Payment");
      ...
      const sumTypeLinkId = await deep.id("${PACKAGE_NAME}", "Sum");
      ...
      const usesTokenTypeLinkId = await deep.id("${PACKAGE_NAME}", "UsesToken");
      ...
      if (initResult.error) {
        ...
        const {error: errorLinkInsertQueryError} = await deep.insert({
          type_id: (await deep.id("${PACKAGE_NAME}", "Error")),
...
await insertHandler({deep, fileTypeLinkId, fileName: 'payInsertHandlerFile', handlerName: 'payInsertHandler', handleName: 'payInsertHandle', triggerTypeLinkId: payTypeLinkId, code, supportsId: dockerSupportsJsId, handleOperationTypeLinkId: handleInsertTypeLinkId, containTypeLinkId, packageId, handlerTypeLinkId, code});
