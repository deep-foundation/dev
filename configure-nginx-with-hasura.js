...
const hasuraDomain = options['--hasura-domain'];
...
if (!deepcaseDomain || !deeplinksDomain || !certbotEmail) {
  ...
  console.log('Usage: node configure-nginx-with-hasura.js --deepcase-domain deep.deep.foundation --deeplinks-domain deeplinks.deep.foundation --hasura-domain hasura.deep.foundation --certbot-email drakonard@gmail.com');
  process.exit(1);
}
...
server {
  ...
  listen 443 ssl;
  server_name ${hasuraDomain};
  ssl_certificate         /etc/letsencrypt/live/${hasuraDomain}/fullchain.pem;
  ssl_certificate_key     /etc/letsencrypt/live/${hasuraDomain}/privkey.pem;
  ssl_trusted_certificate /etc/letsencrypt/live/${hasuraDomain}/chain.pem;
...
(async () => {
  try {
    ...
    await execCommand(`sudo certbot certonly --webroot --webroot-path=/var/www/html -d ${hasuraDomain} -n --agree-tos -m ${certbotEmail}`);
